...
herr_t H5Dget_info(hid_t d_id, H5D_info_t * dataset_info) {
  ...
  if(dataset_info->space_class == H5S_SIMPLE) {
	...
	for(int i = rank - 1; i >= 0; --i) {
	  dims_char_written += sprintf(dataset_info->dims + dims_char_written, "%" PRIu64, (long unsigned int) dims[i]);
	  if(i > 0) {
	  ...
	  if(maxdims[i] == H5S_UNLIMITED) {
	    ...
	    maxdims_char_written += sprintf(dataset_info->maxdims + maxdims_char_written, "%" PRIu64,  (long unsigned int) maxdims[i]);
	  }
	  if(i > 0) {
